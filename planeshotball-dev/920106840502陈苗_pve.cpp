...
double vecMod(double x,double y)
{
    return sqrt(x*x+y*y);
}
//向量的模
...
int aimBall(DataStruct *data,BallStruct &ball,int &leftward,int &rightward)
{
    ...
    modBall=vecMod(ball_v_x,ball_v_y);
    modPlane=vecMod(data->plane1.x-ball_x,data->plane1.y-ball_y);
    cosPosV=vecMul/modBall/modPlane;
    //余弦
    angleCon=(asin(sqrt(1-cosPosV*cosPosV)*vecMod(ball_v_x,ball_v_y)/bul_v)
...
//飞机位置预测
int prePlanePos(DataStruct *data,double &x,double &y)
{
    int time=3000;
    ...
    a_x=-v_x0/vecMod(v_x0,v_y0)*plane_a;
    a_y=-v_y0/vecMod(v_x0,v_y0)*plane_a;
...
//飞机转向
int turnTo(DataStruct *data,double x,double y,int &leftward,int &rightward)
{
    double angleCon;
...
//飞机移动
int moveTo(DataStruct *data,double x,double y,int &forward,int &leftward,int &rightward)
{
    int to,prem=1;
    double pre_x,pre_y,dis;

    dis=vecMod(y-data->plane1.y,x-data->plane1.x);
    if(dis<prem)
    {
        return 1;
    }

    prePlanePos(data,pre_x,pre_y);

    double turn_x,turn_y;
    turn_x=x+data->plane1.x-pre_x;
    turn_y=y+data->plane1.y-pre_y;
    to=turnTo(data,turn_x,turn_y,leftward,rightward);
    if(to==0)
    {
        forward=0;
        return 0;
...
//判断飞机是否移动
int moveIf(DataStruct *data,int &des_x,int &des_y)
{
    int preTime=200;
    ...
    for(int i=0;i<data->ball_size;i++)
    {
        ...
        dis=fabs((k*data->plane1.x-data->plane1.y+C)/vecMod(1, k));
        cross_x=(C2-C)/(1/k+k);
        cross_y=k*cross_x+C;
        ...
        if (dis<(data->plane1.r+ball.r+10)&&t<min_t)
        {
            ...
            des_x=data->plane1.x+ball.r*difx/vecMod(difx,dify);
            des_y=data->plane1.y+ball.r*dify/vecMod(difx,dify);
...
void ai_pve(DataStruct*data,KeyStruct*key){
    ...
    if(moveIf(data,des_x,des_y)||moveFlag==0)
    {
    ...
    if(data->ball_size!=0)
    {
        ...
        for(int i=1;i<data->ball_size;i++)
        {
            ...
            d[i]=vecMod(data->plane1.x-x,data->plane1.y-y);
            k1=v_y/v_x;
        ...
        if(min_d<d0)
        {
            aimBall(data,data->ball[j],key->rotate_left,key->rotate_right);
        }
        //转向100内最近的球射击
        ...
        if(min_d<(data->plane1.r+ball.r)&&flag)
        {
            ...
            {
                moveFlag=moveTo(data,d_x,d_y,key->forward,key->rotate_left,key->rotate_right);
            }
        }
        //备用躲球
